    ...
    useEffect(() => {
        ...
        async function start() {
            try {
                ...
                onError?.(err?.message || t('bookings.cameraError', 'Kamerani ochib bo\u2018lmadi.'));
            }
        }
            ...
            >
                ...
                {!ready && (
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#aaa', fontSize: 13 }}>
                        {t('bookings.cameraLoading', 'Kamera ochilmoqda...')}
                    </div>
                )}
            </div>
            <p style={{ marginTop: 8, color: '#aaa', fontSize: 12, textAlign: 'center' }}>
                {t('bookings.qrHint', 'Mehmonning QR kodini kamera oldida ushlab turing.')}
            </p>
        </div>
    );
}
            ...
            <div className={styles.modalContent} style={{ maxWidth: 440 }}>
                ...
                {!result && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, padding: '0 20px', marginBottom: 4 }}>
                        ...
                        >
                            {t('bookings.checkInByCodeTab', 'Kod bilan')}
                        </button>
                        <button
                            type="button"
                            ...
                            onClick={() => setMode('qr')}
                        >
                            {t('bookings.checkInByQrTab', 'QR orqali')}
                        </button>
                    </div>
                )}
                    ...
                    <div className={styles.modalBody}>
                        ...
                        {mode === 'qr' && busy && (
                            <p style={{ textAlign: 'center', color: '#aaa', fontSize: 13 }}>
                                {t('bookings.checkingIn', 'Tekshirilmoqda...')}
                            </p>
                        )}
