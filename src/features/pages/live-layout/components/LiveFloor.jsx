...
export default function LiveFloor() {
    ...
    // FIX: `currentFloor?.zones || []` built a brand-new array literal on
    // every render whenever the floor had no zones, so each useMemo below saw
    // a "changed" dependency and recomputed on every single render.
    const zones = useMemo(() => currentFloor?.zones || [], [currentFloor]);
